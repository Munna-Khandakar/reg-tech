...
function ReunionAllUsers() {
  ...
  useEffect(() => {
    const getPosts = async () => {
      const res = await fetch(`/api/reunion/users/1`);
  ...
  const fetchPosts = async () => {
    ...
    const res = await fetch(`/api/reunion/users/${page}`);
  ...
  const searchQueryHandler = async () => {
    ...
    const res = await fetch(`/api/find/reunion/${searchQuery}`);
    const data = await res.json();
    setPlateData(data);
    setHasMore(false);
    setSearchQuery("");
...
export default ReunionAllUsers;
